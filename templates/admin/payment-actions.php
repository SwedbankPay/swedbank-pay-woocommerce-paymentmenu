...
?>
<div>
	<strong><?php _e( 'Payment Info', 'swedbank-pay-woocommerce-paymentmenu' ); ?></strong>
	<br />
	<?php if ( isset( $info['paid']['number'] ) ) : ?>
		<strong><?php _e( 'Number', 'swedbank-pay-woocommerce-paymentmenu' ); ?>
			:</strong> <?php echo esc_html( $info['paid']['number'] ); ?>
		<br/>
	<?php endif; ?>
	<?php if ( isset( $info['paid']['instrument'] ) ) : ?>
		<strong><?php _e( 'Instrument', 'swedbank-pay-woocommerce-paymentmenu' ); ?>
			: </strong> <?php echo esc_html( $info['paid']['instrument'] ); ?>
		<br/>
	<?php endif; ?>
	<?php if ( isset( $info['paid']['transactionType'] ) ) : ?>
		<strong><?php _e( 'Transaction type', 'swedbank-pay-woocommerce-paymentmenu' ); ?>
	...
	<?php if ( $gateway->api->can_capture( $order ) ) : ?>
		<button id="swedbank_pay_capture"
			...
			<?php _e( 'Capture Payment', 'swedbank-pay-woocommerce-paymentmenu' ); ?>
		</button>
	<?php endif; ?>
	...
	<?php if ( $gateway->api->can_cancel( $order ) ) : ?>
		<button id="swedbank_pay_cancel"
				data-nonce="<?php echo esc_attr( wp_create_nonce( 'swedbank_pay' ) ); ?>"
				data-order-id="<?php echo esc_html( $order->get_id() ); ?>">
			<?php _e( 'Cancel Payment', 'swedbank-pay-woocommerce-paymentmenu' ); ?>
		</button>
	<?php endif; ?>
</div>
